...
1. **Введение и Архитектура** — проблематика, цели, стек, схема БД, карта API
2. **Инфраструктура и Настройка** — инициализация проекта, миграции, модели
3. **Реализация Core API** — контроллеры, валидация, Resources, роуты
4. **Безопасность и Доступ** — Sanctum, политики, роли, ограничения
5. **Продвинутые фичи** — уведомления, очереди, оптимизация
6. **Итоги и Деплой** — тесты, документация, деплой в production

---

## Цели вебинара

1. **Понять архитектуру** — от проблематики до схемы БД и контракта REST API биржи event-услуг
2. **Реализовать ключевые части** — аутентификация (Sanctum), каталог услуг, заказы, роли заказчик/исполнитель
3. **Применить практики** — валидация, API Resources, Policies, тесты, версионирование API

**Смысл:** дать целостную картину: как от постановки задачи перейти к рабочему, безопасному и поддерживаемому API на Laravel, без лишней теории и с фокусом на то, что пригодится в реальных проектах.
...
1. **Прошли путь от задачи до API** — от формулировки проблемы и целей до схемы БД, миграций и эндпоинтов.
2. **Реализовали REST API на Laravel** — версионирование (v1), валидация, API Resources; познакомились с картой эндпоинтов и форматом ответов.
3. **Разобрали безопасность** — аутентификация через Sanctum, авторизация через Policies, роли заказчик/исполнитель, throttle и валидация входных данных.
4. **Познакомились с инфраструктурой** — миграции, тесты, документация, единый стиль ответов и подготовка к деплою.
5. **Затронули масштабирование** — индексы БД, eager loading, очереди и кэширование конфига и маршрутов.
